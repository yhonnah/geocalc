
def sphereFormula(): 

    # This file was generated by the Tkinter Designer by Parth Jadhav
    # https://github.com/ParthJadhav/Tkinter-Designer

    from main_menu import openShapesWindow
    from pathlib import Path
    from math import pi
    # from tkinter import *
    # Explicit imports to satisfy Flake8
    from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage


    OUTPUT_PATH = Path(__file__).parent
    ASSETS_PATH = OUTPUT_PATH / Path(r"Calculator GUI/build/assets/frame0")


    def relative_to_assets(path: str) -> Path:
        return ASSETS_PATH / Path(path)

    def sphereRadius(r):
        sphereDiameter = 2 * r
        circumference = pi * sphereDiameter

        sphereArea = 4 * pi * (r ** 2)
        sphereVolume = (4/3) * pi * (r ** 3)

        return sphereArea, sphereVolume, sphereDiameter, circumference

    def calculateSphere():
        try:
            # Get the input radius from entry_1 widget
            radius = float(entry_1.get())

            # Calculate sphere properties
            area, volume, diameter, circumference = sphereRadius(radius)

            with open("sphere_results.txt", "w") as file:
                file.write(f"Surface Area: {area}\nVolume: {volume}\nDiameter: {diameter}\nCircumference: {circumference}")
            
            # Format the results to display two decimal points
            area = "{:.2f}".format(area)
            volume = "{:.2f}".format(volume)
            diameter = "{:.2f}".format(diameter)
            circumference = "{:.2f}".format(circumference)

            # Display the results in the text widget
            result_text = f"Surface Area: {area}\nVolume: {volume}\nDiameter: {diameter}\nCircumference: {circumference}"
            result_display.config(state="normal")
            result_display.delete(1.0, "end")
            result_display.insert("end", result_text)
            result_display.config(state="disabled")

        except ValueError:
            # Handle invalid input (non-numeric)
            result_display.config(state="normal")
            result_display.delete(1.0, "end")
            result_display.insert("end", "Invalid input: Please enter a number.")
            result_display.config(state="disabled")

    sphereWindow = Tk()

    sphereWindow.geometry("862x519")
    sphereWindow.configure(bg = "#016938")


    canvas = Canvas(
        sphereWindow,
        bg = "#016938",
        height = 519,
        width = 862,
        bd = 0,
        highlightthickness = 0,
        relief = "ridge"
    )

    canvas.place(x = 0, y = 0)
    canvas.create_text(
        334.0,
        33.0,
        anchor="nw",
        text="Sphere Calculator",
        fill="#FCFCFC",
        font=("Roboto Bold", 24 * -1)
    )

    image_sphere = PhotoImage(
        file=relative_to_assets("sphere.png"))
    sphere = canvas.create_image(
        431.0,
        245.0,
        image=image_sphere
    )

    entry_sphere = PhotoImage(
        file=relative_to_assets("entry_1.png"))
    entry_bg_1 = canvas.create_image(
        145.0,
        220.0,
        image=entry_sphere
    )
    entry_1 = Entry(
        bd=0,
        bg="#FFFFFF",
        fg="#000716",
        highlightthickness=0
    )
    entry_1.place(
        x=49.0,
        y=183.0,
        width=192.0,
        height=72.0
    )

    canvas.create_text(
        72.0,
        140.0,
        anchor="nw",
        text="Input radius",
        fill="#FFFFFF",
        font=("Inter", 24 * -1)
    )

    canvas.create_rectangle(
        342.0,
        429.0,
        519.0,
        487.0,
        fill="#FFFFFF",
        outline="")

    canvas.create_rectangle(
        757.0,
        25.0,
        825.0,
        64.0,
        fill="#FFFFFF",
        outline="")

    button_sphere = PhotoImage(
        file=relative_to_assets("button_1.png"))
    button_1 = Button(
        image=button_sphere,
        borderwidth=0,
        highlightthickness=0,
        command=calculateSphere,
        relief="flat"
    )
    button_1.place(
        x=363.0,
        y=436.0,
        width=128.0,
        height=43.0
    )

    button_image_2 = PhotoImage(
        file=relative_to_assets("button_2.png"))
    button_2 = Button(
        image=button_image_2,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: (sphereWindow.destroy(), openShapesWindow()),
        relief="flat"
    )
    button_2.place(
        x=757.0,
        y=25.0,
        width=68.0,
        height=43.0
    )

    result_display = Text(
        bd=0,
        bg="#FFFFFF",
        fg="#000716",
        highlightthickness=0,
        wrap="word"
    )
    result_display.place(
        x=600,
        y=432.0,
        width=177.0,
        height=58.0
    )

    result_display = Text(
        bd=0,
        bg="#FFFFFF",
        fg="#000716",
        highlightthickness=0,
        wrap="word",
        font=("Inter", 13)
    )
    result_display.place(
        x=600,
        y=432.0,
        width=177.0,
        height=58.0
    )

    sphereWindow.resizable(False, False)
    sphereWindow.mainloop()
